...
<div class="panel-heading panel-Info" style="margin-top: 0px">
				<h3 class="panel-title" align="center"><b>&darr; Status of Travel Requests after Budget Approval&darr;</b> </h3>
			</div>
<div class="panel-body">
<table width="1080" border="1">
  <tr>
    <td width="230">&nbsp; Employee </td>
...
?>	
  ...
  <tr>
    ...
    <td width="100" class="form-control">
      
    <select id="stat" name="stat" value=<?php echo $wk_status; ?>>
      <option class="form-control" value="booking">Create Booking</option>
      <option class="form-control" value="view">View Traveling</option>
      
      </select>
      
	</td>
    <td width="150"><label class="form-control">
      <input type="hidden" name="tripid" value="<?php echo $trip_id; ?>">
      <input type="submit" class="btn btn-lg btn" name="submit" value="submit"> </label></td>
	</form>
...
{   
  ...
  echo $trip_id = $_POST['tripid'];

   //$sql=mysqli_query($link, "UPDATE employee_trips SET status='$stat', date_updt=NOW() WHERE trip_id='$trip_id'") or die(mysqli_error($link));

  }

  if ($stat=="view" OR $stat=="booking") {
     header("Location: travel_booking.php?user=$user& stat=$stat& tripid=$trip_id");
  }
?>
  </tr>
  </table>
